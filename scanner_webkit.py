...
# 🚀 WebKit 极其轻量，并发可以尝试拉到 60-80
MAX_CONCURRENT = config.get("max_concurrent_pages", 60) 
            ...
            try:
                ...
                if status == 'waf':
                    ...
                    if not (class_name in invalid_marks and school in invalid_marks):
                        ...
                        results.append(line)
                        print(f"✅ [WebKit解析] P{process_id}-C{coro_id:02d} | {cid} | 🏫 {school} | 🧑‍🏫 {teacher} | 🎓 {class_name}", flush=True)
...
def main():
    ...
    print(f"🚀 [WebKit 苹果引擎极速版] 启动！彻底脱离 Chromium 生态！", flush=True)
    print(f"⚙️ 分配: {process_count}核 ✕ 每核 {coros_per_process} 并发 = {process_count * coros_per_process} 并发", flush=True)
    ...
    try:
        while any(p.is_alive() for p in processes):
            ...
            if now - last_print >= 5: 
                ...
                print(f"\n🔥 [满血监控] 完成: {c}/{total_tasks} ({pct:.2f}%) | ⚡ WebKit时速: {speed:.1f} 个/秒 | ⏳ 剩余: {eta}")
